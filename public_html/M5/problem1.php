...
function processBirds($birds) {
    ...
    // Start edits
    // jd755 10/16/24
    $subArrNum = 0;// variable to interate through $subset sub-arrays
    foreach($birds as $subArr) // for loop that goes through each subarray in $birds and takes out the specific data
    {
        $subset[$subArrNum]['name'] = $subArr['name'];
        $subset[$subArrNum]['color'] = $subArr['color'];
        $subset[$subArrNum]['region'] = $subArr['region'];
        $subArrNum++;
    }
